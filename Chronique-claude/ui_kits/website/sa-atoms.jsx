...
function CinemaIntro({ onEnter }) {
  ...
  const scrollZoom = lerp(1.42, 48, easeRush(clamp01((p - 0.34) / 0.34)));
  const preTailZoom = lerp(1.42, 12, easeRush(clamp01((p - 0.34) / 0.24)));
  const zoom = reduced
    ? (p < 0.34 ? zoomBase : scrollZoom)
    : (p < 0.34 ? zoomBase : (displayedTailP > 0 ? lerp(12, 48, displayedTailP) : preTailZoom));
  const logoOp = 1;
    ...
    <section ref={trackRef} className="cinema">
      <div className="cinema__stage">
        ...
        <div className="cinema__logo"
          ...
          <img src={window.__r('restaurant_chronique_svg')} alt="Restaurant la chronique" />
        </div>
